...
public class ShootingView extends SurfaceView
    ...
    public ShootingView(Activity activity) {
        ...
        firstFont = Typeface.createFromAsset(getContext().getAssets(), "7SegmentsLED.ttf");
        nikoFont = Typeface.createFromAsset(getContext().getAssets(), "nicomoji_1.00.ttf");
    ...
    public void run() {
        ...
        while (thread != null) {
            ...
            g.setScoreTypeface(firstFont);
            g.drawText(num2str((int)offsetTime,2),
                    400, 40+g.getOriginY() - (int) g.getFontMetrics().ascent);
            drawScore();
            /*
            g.setTextSize(30);
            g.setScoreTypeface(firstFont);
            g.drawText(num2str(score, 6),
                    130 + g.getOriginX(), 20 - (int) g.getFontMetrics().ascent);
            g.setTextSize(30);
            g.setScoreTypeface(nikoFont);
            g.drawText("SCORE ", 10 + g.getOriginX(), 20 - (int) g.getFontMetrics().ascent);
            */
            g.unlock();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (touchAction == MotionEvent.ACTION_DOWN) {
            ...
            //プレイ
                ...
                //弾の追加(3)
                shots.add(new Point(shipX - 30, shipY - 50));
